...
export class UsersService {
  ...
  async createUser(q: QueryRunner, createUserDto: CreateUserDto) {
    await this.verifyUsernameAndEmail(
      q,
  ...
  async findOne(q: QueryRunner, id: string) {
    const user = await q.manager.findOne(User, { where: { id: id } });
    if (!user) throw NotFound;
  ...
  async findUserByUsername(q: QueryRunner, username: string) {
    const user = await q.manager.findOne(User, {
  ...
  async verifyUsernameAndEmail(
    q: QueryRunner,
    username: string,
    email: string,
  ) {
    if (username) {
      const isNotEmail = await q.manager.findOne(User, {
